...
namespace Utility
{
	public class ComponentPool<T> : MonoBehaviour, IObjectPool<T> where T : Component
	{
		...
		private void Awake()
		{
			...
			pool = isSafe ? new SafeObjectPool<T>(OnCreate, poolParameters)
								.SetOnReceive(OnReceive)
								.SetOnRelease(OnRelease)
								.SetOnClear(OnDestroy) :
							new ObjectPool<T>(OnCreate, poolParameters)
								.SetOnReceive(OnReceive)
								.SetOnRelease(OnRelease)
								.SetOnClear(OnDestroy);
		}
		...
		protected virtual void OnDestroy(T that)
		{
			if(fastDestroy) {
